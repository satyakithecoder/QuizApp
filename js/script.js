...
window.onload = async function() {
    try {
		console.log(random)
        const response = await fetch("https://opentdb.com/api.php?amount=20&category=9&difficulty=medium&type=multiple");
		...
		const allAnswers = [data.results[0].correct_answer].concat(data.results[0].incorrect_answers)
